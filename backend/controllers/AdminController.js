...
export const Login = async(req, res) => {
    try {
        ...
        if(!match) return res.status(200).json({msg: "not Matched"});
        if(match) return res.status(200).json({msg: "Matched"});
        ...
        await Adminloging.update({refresh_token: refreshToken},{
            where:{
                id: adminId
            }
        });
        ...
        res.status(200).json({msg:"not Matched"});
    }
}
...
export const Logout = async(req, res) => {
    ...
    const adminId = admin[0].id;
    await Adminloging.update({refresh_token: null},{
        where:{
            id: adminId
        }
    });
